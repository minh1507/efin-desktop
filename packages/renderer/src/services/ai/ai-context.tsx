...
import { initOpenAI, AIModelConfig } from './openai-service';
...
export const AIProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (apiKey && modelConfig.type === 'openai') {
      try {
        initOpenAI(apiKey);
        setModelConfig(prev => ({ ...prev, apiKey }));
      } catch (error) {
        console.error('Failed to initialize OpenAI:', error);
      }
    }
    
    // For custom models, just update the apiKey in the config
    if (apiKey && modelConfig.type === 'custom') {
      setModelConfig(prev => ({ ...prev, apiKey }));
    }
  }, [apiKey]);
